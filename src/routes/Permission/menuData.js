...
@Form.create()
export default class menuDataPage extends PureComponent {
  state = {
    ...
    return (
      <Form onSubmit={this.handleSubmitQueryForm} layout="inline">
        <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
          ...
          <Col md={8} sm={24}>
            <FormItem label="是否删除">
              {getFieldDecorator('suIsDeleted', { initialValue: 'false' })(
  ...
  render() {
    const { menuData: { loading: menuDataLoading, data, viewData } } = this.props;
    ...
    const columns = [
      ...
      {
        title: '创建时间',
        dataIndex: 'suGmtCreate',
      },
      {
        title: '更新时间',
        dataIndex: 'suGmtModified',
      },
      {
        title: '是否删除',
        dataIndex: 'suDeleted',
