...
const tableId = 'mtId';
const tableDelete = 'mtIsDeleted';

@connect(state => ({
  templatePage: state.templatePage,
}))
@Form.create()
export default class TemplatePage extends PureComponent {
  state = {
  ...
  renderSimpleQueryForm() {
    const { templatePage: { unitSelectData } } = this.props;
    const unitOptions = [];
    if (unitSelectData) {
...
// 渲染复杂查询
        ...
        <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
          ...
          <Col md={8} sm={24}>
            <FormItem label="发送用户外键">
              {getFieldDecorator('mtSuerId')(
                <Input placeholder="请输入" />
              )}
            </FormItem>
          </Col>
          <Col md={8} sm={24}>
            <FormItem label="发送用户行政区外键">
              {getFieldDecorator('mtAreaId')(
                <Input placeholder="请输入" />
              )}
            </FormItem>
          </Col>
          <Col md={8} sm={24}>
            <FormItem label="发送用户单位外键">
              {getFieldDecorator('mtUnitId')(
                <Input placeholder="请输入" />
              )}
            </FormItem>
          </Col>
          ...
          <Col md={5} sm={15}>
            <FormItem label="模板状态">
              {getFieldDecorator('mtState', { initialValue: '1' })(
                <Select placeholder="请选择" style={{ width: '100%' }}>
  ...
  render() {
    const { templatePage: { loading: templatePageLoading, data, viewData } } = this.props;
    const { selectedRows, addModalVisible, addOrUpdate, key } = this.state;
    // templatePage的columns
    ...
    const detailColumns = [
      {
        title: '创建时间',
        dataIndex: 'mtGmtCreate',
        sorter: true,
        render: val => <span>{moment(val).format('YYYY-MM-DD HH:mm:ss')}</span>,
      },
      {
        title: '最后操作时间',
        dataIndex: 'mtGmtModified',
        sorter: true,
        render: val => <span>{moment(val).format('YYYY-MM-DD HH:mm:ss')}</span>,
      },
    ];
