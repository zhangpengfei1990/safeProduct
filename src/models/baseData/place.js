...
import { viewPlace, queryPlace, removePlace, addPlace, updatePlace, queryAreaSelect, queryExamPlanNameSelect } from '../../services/api';
  ...
  effects: {
    ...
    *view({ payload, callback }, { call, put }) {
      ...
      if (response.status === '1') {
        yield put({
          type: 'viewSave',
          payload: response.dataMain.dpPlaceVo,
        });
      } else {
        message.error(response.msg);
      }
